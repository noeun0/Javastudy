...
public class ConstantDemo {
    ...
    //과일 APPLE 과 기업 APPLE이 서로 같은 이름을 갖는다.프로그램은 오동작 할것이다. 하지만 FINAL로 선언 했기 때문에 컴파일이 되지 않고 이름이 중복되는 문제는 막을 순 있다.
...
if(FRUIT.APPLE==COMPANY.APPLE){
	Sysrem.out.println("과일애플과 기업애플은 같습니다");
}
```

: 실제로는 비교할 수 없는 대상을 비교할 수 있다.  같은 데이터 타입

-> 서로 비교조차 할 수 없게 하려면? (구동과정에서 오류가 생기면 안되기 때문)
...
class Fruit{
    ...
    // 클래스 변수로 해당 클래스의 인스턴스를 사용하고 있다. 
    //final이기 때문에 불변이고, static이므로 인스턴스로 만들지 않아도된다.
...
public class ConstantDemo {
    ...
    public static void main(String[] args) {
       if(Fruit.APPLE == Company.APPLE) {// 오류가 난다. 
...
타입이 달라서 비교불가. 비교조차 금지하게 되어 오류가 나게함. 

-> 문제가 있는 부분을 컴파일러가 발견할 수 있어서 실행 전에 오류를 찾아낼 수 있다. 



: 하지만 위의 두 코드는 두가지 문제점이 있다.

1. switch문에서 사용할 수 없다.
   - 원시 데이터 타입 : byte, short, char, int, enum, String, Charactor, Byte, Short, Integer
2. 선언이 너무 복잡하다.

# enum

열거형 enumerated type 열거형은 서로 연관된 상수들의 집합
...
enum Company2{
    GOOGLE, APPLE, ORACLE; // 다른 이넘 클래스의 애플과 다른 것.
}
...
* 키원드 enum을 사용하기 때문에 구현의 의도가 열거임을 분명하게 나타낼 수 있다.
...
> enum은 사실상 클래스이기 때문에 생성자를 갖을 수 있다.
...
```
Call ContstructorAPPLE
Call ContstructorPEACH
Call ContstructorBANANA
57 kcal
```

Call Constructor 가 출력된 것은 생성자 Fruit가 호출되었음을 의미한다. 이것이 3번 호출되었다는 것은 필드의 숫자만큼 호출되었다는 뜻이다. 즉, enum은 생성자를 갖을 수 있다.



하지만 아래와 같이 바꾸면 에러가 발생
...
> enum의 생성자가 접근 제어자 private만 허용한다.  덕분에 Fruit을 직접 생성할 수 없다. 
>
...
enum Fruit2{
    ...
    //상수를 선언과 동시에 생성자를 호출하고 매개변수로 색상값을 전달하였다. 
    public String color; //필드 생성
    Fruit2(String color){
        this.color = color;
        System.out.println("Call Contstructor"+this);
    }
...
> 클래스방식과 달리 어떠한 데이터가 있는지 몰라도. 하나씩 꺼내서 처리할 수 있다.
...
public class ConstantDemo2 {
    ...
    public static void main(String[] args) {
        for(Fruit2 f : Fruit2.values()) { // fruit2의 상수들을 열거해서 f변수에 차례대로 넣어서 for문을 사용할 수 있다.
...
1. 연관된 상수 들을 저장. 변경되지 않게 고정
2. 내부에 생성자, 필드, 메소드 등을 갖을 수 있다.  클래스에서 할 수 있는 기능들을 구현 가능
3. 상수들을 담은 배열을 갖을 수 있어서 하나하나 꺼내서 사용할 수 있다. 
